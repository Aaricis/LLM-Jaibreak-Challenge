...
# Batch sizes for RTX 4090 (24 GB).
# ChatLM 已改为逐条生成，不受此参数影响。
# Guard (0.6B) 和 Relevance (1.7B) 使用此 batch size。
BATCH_SIZE = 8
...
def _get_common_args():
    ...
    parser.add_argument(
        ...
        help=f"Batch size for guard and relevance models. Default: {BATCH_SIZE}. Lower if OOM.",
    )
...
def main():
    ...
    print(f"Guard/Relevance batch size : {args.batch_size}")
    print(f"ChatLM                     : sequential (preserves original scoring)")
    ...
    for batch_start in range(0, total_todo, bs):
        ...
        # batch_judge 内部：
        #   guard      → batch  (do_sample=False, 与原版一致)
        #   chatlm     → 逐条   (do_sample=True,  与原版完全相同)
        #   relevance  → batch  (do_sample=False, 与原版一致)
        ...
        print(f"Processed {done} / {total_todo}  "
