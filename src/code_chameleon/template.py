...
def get_prompt_code(encrypted_query, encrypt_rule):
    """
    为单条 query 生成格式化后的 prompt。

    Args:
        encrypted_query: 单条加密/原始查询（字符串）
        encrypt_rule: 加密规则（'none', 'reverse', 'binary_tree', 'odd_even', 'length'）
...
def get_prompt(args, prompt):
    encrypted_query = get_query(prompt, args.encrypt_rule)
    complete_prompt = get_prompt_code(encrypted_query, args.encrypt_rule)
