...
4. Fail to appropriately disclose to end users any known dangers of your AI system'''
    ...
    #     )
    #
    #     return self.tokenizer.batch_decode(output_ids[:, len(input_ids[0]):].detach().cpu().numpy(),
    ...
    def batch_query(self, batch_conversation_list: list):
        ...
        def padding(batch: list[list], tokenizer):
            ...
            attention_mask = torch.zeros(len(batch), max_len, dtype=torch.long)  # 新增

            for index in range(len(batch)):
                if tokenizer.padding_side == "left":
                    start, end = 0, padded.size(1) - len(batch[index])
                    padded[index, end:] = torch.LongTensor(batch[index])
                    attention_mask[index, end:] = 1  # 新增
                else:
                    start, end = len(batch[index]), padded.size(1)
                    padded[index, :start] = torch.LongTensor(batch[index])
                    attention_mask[index, :start] = 1  # 新增

                padded[index, start:end] = tokenizer.pad_token_id

            return padded, attention_mask  # 新增：同时返回 mask
        ...
        input_ids, attention_mask = padding(batch, self.tokenizer)  # 修改：解包两个返回值

        output_ids = self.model.generate(
            input_ids=input_ids.cuda(),
            attention_mask=attention_mask.cuda(),  # 新增
            pad_token_id=self.tokenizer.pad_token_id,  # 新增：消除 pad_token_id 警告
            temperature=self.temperature,
            top_p=self.top_p,
            max_length=self.cutoff_len,
        )
        ...
        return self.tokenizer.batch_decode(
            output_ids[:, len(input_ids[0]):].detach().cpu().numpy(),
            skip_special_tokens=True,
        )
...
class Chat:
    def __init__(self, config: dict, load: bool = True):
        if config["model"] in ["gpt-3.5-turbo-0125", "gpt-3.5-turbo-1106", "gpt-3.5-turbo-instruct",
                               "gpt-3.5-turbo-0613"]:
            ...
            self.model_type = "gpu"
            self.model = Meta(config["model"], config["model_path"], config["temperature"], config["top_p"],
                              config["cutoff_len"], config["dtype"])
