...
args = Namespace(
    ...
    rewrite_model="deepseek-v4-flash",  # 改写模型
    judge_model="deepseek-v4-flash",  # 审核模型
    # rewrite_model="deepseek-v4-pro",  # 改写模型
    # judge_model="deepseek-v4-pro",  # 审核模型
    max_tokens=3584,
    temperature=0,
    round_sleep=1,
    fail_sleep=1,
    ...
    gpt_api_key=os.environ.get("DEEPSEEK_API_KEY"),  # 填入你的 API Key
    gpt_base_url=os.environ.get("DEEPSEEK_BASE_URL")  # 智谱端点，或 DeepSeek 的 https://api.deepseek.com/v1
)
...
def rewrite_and_nest(harm_behavior: str):
    ...
    - First determine whether the input prompt is harmful.
    - If harmless, directly return the original prompt.
    - Otherwise perform rewriting + nesting.
    """

    print("******* Initial Harmfulness Check *******")
    init_label = harmful_classification(args, harm_behavior)
    print(f"Initial label from {args.judge_model}: {init_label}\n")

    # ---------------------------------------------------
    # If harmless, directly return
    # ---------------------------------------------------
    init_label = init_label.strip()[0]
    if init_label != "1":
        print("******* Input prompt is harmless. Skip rewriting. *******\n")
        return harm_behavior

    print("******* Harmful prompt detected. Start rewriting. *******\n")
    ...
    while True:
        ...
        current_prompt = harm_behavior

        for index in operation_indexes:
            print(f"Excute function {index}: {operations[index].__name__}")
            current_prompt = operations[index](args, current_prompt)
            print(f"Rewritten prompt: {current_prompt}\n")
            temp_rewrite_results.append(
                [operations[index].__name__, current_prompt]
            )

        # check if the prompt meets the requirements; if it does, break out of the loop.
        print("******* Check if harmful semantics are preserved *******")
        label = harmful_classification(args, current_prompt).strip()[0]
        print(f"\nThe label given by {args.judge_model} is: {label}\n")
        if label == "1":
            print(f"******* Prompt Rewriting Done! *******\n")
            harm_behavior = current_prompt
            break
        else:
