...
def get_data_list(data_name: list[str] | str) -> dict:
    '''Extract data (requires run from project root)
    ...
    prompt, template = get_data_list(['h_custom', 'b_custom', 'harmful_behaviors', 'our_template'])  # our_template is jailbreak template data
    # return a dict {'data_list': [...], 'template_list': [...], 'asr_list': [...]}
    ```
    '''
    ...
    for name in data_name:
        ...
        if name in template_data_name:
            ...
            with open(f'src/xJailbreak/data/{attr}/{name}.{file_type}', 'r', encoding='utf-8') as f:
